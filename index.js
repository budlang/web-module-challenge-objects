...
burger.discount = function discount(customer)
 if(customer === 'teacher' || custoemr === 'student'){
...
reviews.push(name:"Reyna", rating: 3.5, feedcak: "This restuant is awesome. Dont listen to the others"})
...
function changeRevieww(arr, index, feedback){
 arr[index[.feedback = feedvback;
 return arr;]]
}
changeRevieww(review, 6, "this place is chill with really cool people, great for getting work done on weekdays");
...
function getReviewByIndex(reviews, index) {
  const numberReview = reviews[index]
 return `${reviews.name} gave the resturant a ${`review.rating}, and their was: $reviews.feedback}`;
 }
 console.log('Task 7 ---->', getReviewsByIndex(reviews, 4));
...
function getLastReview(arr) {
   ...
   return `${review.name} gave the restarant a ${review.rating` and their feedback was ${last.feedback};
 }
console.log(getLastRview(review));
